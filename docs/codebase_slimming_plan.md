...
| Path | Category | Current Role | Keep / Move / Archive | Reason | Risk |
|---|---|---|---|---|---|
| `outputs/demo/*.json,*.jsonl` | CORE | Committed fallback fixtures (`--offline-fixture`). | KEEP. | Demo safety net. | high. |
...
| Path | Category | Current Role | Keep / Move / Archive | Reason | Risk |
|---|---|---|---|---|---|
| `traces/*.jsonl` | LEGACY | Pre-orchestrator trace samples. | KEEP for now; mark deprecated in `docs/data_format.md`; move to `archive/traces/` post-demo. | Canonical traces now live in `outputs/demo/` and `live-data/snapshots/`. | low — nothing imports these. |
...
7. Rename `outputs/demo/` → `outputs/_demo_fixtures/` (clarifies fixture vs build).
