...
| Seed-meaningful simulator runs | Implemented | `tools/workload_jitter.py`; `tests/test_workload_jitter.py`; orchestrator materialises `outputs/.../workload_instance.json` for the simulator backend | jitters arrival/burst MAGNITUDES (counts + structure preserved) per seed → different seeds give different metrics (multi-seed stats). xv6 stays deterministic-by-profile (curated C tables, no PRNG) |
| Dashboard 🎲 random workload | Implemented | `dashboard_live/src/components/RunControls.jsx` | rolls a fresh profile + seed each RUN; on simulator that re-jitters the instance, on xv6 only the profile changes |
| LLM burst-prior ablation | Implemented | `experiments/burst_ablation.py`; `tests/test_burst_ablation.py`; report at `outputs/ablation/`; surfaced on the dashboard via `LLMContributionCard.jsx` | scores LLM prior vs EMA-cold vs fixed heuristic on held-out `actual_bursts`. Finding: LLM wins **ordering** (≈0.90 vs 0.50 blind / 0.72 heuristic — the metric SJF/SRTF use) but overshoots **magnitude** (high MAE), which the kernel EMA refines. LLM ranks at cold-start, EMA calibrates. `actual_bursts` used evaluator-side only |
| Multi-seed robustness sweep | Implemented | `tools/seed_sweep.py`; `tests/test_seed_sweep.py`; reports at `outputs/seed_sweep/` | runs a workload across N seeds (each a jittered instance) → per-algorithm target metric **mean ± std** + best-algorithm frequency + ranking stability. Turns single-run anecdotes into statistics. Reuses the orchestrator profile-alias map; simulator-only (xv6 is deterministic-by-profile) |
